...
export class TrackingService {
  ...
  async getDriverLocation(driverId: string): Promise<DriverLocation> {
    try {
      ...
      this.logger.error(
        `Error getting driver location: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
  ...
  private async getDriverLocationInternal(driverId: string): Promise<DriverLocation> {
    ...
    const inMemoryLocation = this.locationProcessingService.getDriverLocation(
      driverId,
    ) as InMemoryLocation | null;
   ...
   */
  private mapInMemoryLocationToDriverLocation(
    driverId: string,
  ...
  async getDriverLocationHistory(
    ...
    try {
      const locations = await this.driverLocationRepository.getLocationHistory(
        driverId,
        startTime,
        endTime,
        limit,
      ...
      this.logger.error(
        `Error getting driver location history: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
  ...
  async findNearbyDrivers(
    ...
    try {
      return await this.driverLocationRepository.findNearbyDrivers(
        latitude,
        longitude,
        radiusMeters,
        limit,
      ...
      this.logger.error(
        `Error finding nearby drivers: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
  ...
  async getOrderTracking(orderId: string): Promise<OrderTracking> {
    try {
      ...
      this.logger.error(
        `Error getting order tracking: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
   ...
   */
  private async getDeliveryAndDriverLocation(
    orderId: string,
  ): Promise<[ActiveDelivery, DriverLocation]> {
   ...
   */
  private createOrderTrackingResponse(
    ...
    return {
      ...
      destination: this.createLocationPoint(
   ...
   */
  private createDriverLocationPoint(location: DriverLocation): {
    latitude: number;
    longitude: number;
    heading?: number;
    updatedAt: Date;
    ...
    return {
      ...
      heading: location.heading,
      updatedAt:
        typeof location.timestamp === 'string' ? new Date(location.timestamp) : location.timestamp,
    };
  }
  ...
  async createActiveDelivery(
    ...
    try {
      ...
      this.logger.error(
        `Error creating active delivery: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
  ...
  async updateDeliveryStatus(deliveryId: string, status: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error updating delivery status: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
